...
import (
	...
	usecases "clean_architecture/golang/usecases"
...
func New(client firebaseClient) usecases.FirebaseHandler {
...
func (tH tokenHandler) EmailSignInLink(email string) (*string, error) {
	actionCodeSettings := &auth.ActionCodeSettings{URL: fmt.Sprintf("%s/verify_email", viper.GetString("server.fronthost"))}
	link, err := tH.client.EmailSignInLink(context.Background(), email, actionCodeSettings)
	if err != nil {
